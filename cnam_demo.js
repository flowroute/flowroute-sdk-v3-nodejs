...
account_cnams.then(function(response) {
  ...
  if (cnams_list.length == 0) {
    ...
    create_cnam.then(function(response) {
      ...
      console.log("Cannot proceed with demo without a valid CNAM.  Please ask or wait for CNAM to be approved and try again.")
      process.exit();
    }, function(err) {
    ...
    /*
    // An example of how to delete a CNAM if you want to go through the process of getting another approved
    var delete_cnam = cnams_controller.deleteACNAMRecord(cnamId, callback)
